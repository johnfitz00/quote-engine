...
@Service
class RatingEngine(
    private val rateLoader: RatingDataLoader,
) {
    companion object {
    ...
    fun calculatePremium(request: RatingRequest): RatingResult {
        ...
        for ((type, value) in factorInputs) {
            val row =
                rateLoader.findApplicable(type, value, date).singleOrNull()
        ...
        val stampDutyRate =
            STAMP_DUTY_RATES[request.state.uppercase()]
                ?: throw RatingException(
